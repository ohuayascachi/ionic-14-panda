...
export class MyProdListComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.products$ = this.productService.getProductsByUser();
  ...
  selectProdByCategoria(e: any) {
    ...
    this.products$ = this.productService.getProductsByUser().pipe(
    ...
    if (e.target.value === null || e.target.value === undefined || !e.target.value) {
      ...
      subca$.pipe(map(resp => resp[0])).subscribe((res: SubcategoriaGet) => {
        ...
        this.products$ = this.productService.getProductsByUser()
          .pipe(
            map(resp => resp
              .filter(x => x.categoria === res.categoria && x.subcategoria === res.id)));
